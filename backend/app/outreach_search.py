...
async def generate_search_queries_matrix(
    ...
    is_tender_or_trading = any(
        marker in prompt.lower()
        for marker in [
            "44-фз", "223-фз", "тендер", "закупк", "снабжен", "посредник", "торгов",
            "комплексн", "тмц", "по тз", "подбор", "исключить завод", "без собственного"
        ]
    )

    if is_tender_or_trading:
        if is_extend or wave_index > 1:
            system_prompt = (
                f"Ты ведущий эксперт по B2B поиску компаний комплексного снабжения, торговых домов и операторов закупок. "
                f"ЭТО РЕЖИМ ДОБОРА (ВОЛНА №{wave_index}). В базе уже собрано {existing_count}+ организаций по этой теме. "
                "КРИТИЧЕСКИ ВАЖНО: Целевая аудитория — ТОЛЬКО компании комплексного снабжения, торговые дома, комплектовщики объектов и подрядчики по 44-ФЗ/223-ФЗ, которые закупают продукцию под ключ по ТЗ заказчиков. "
                "СТРОЖАЙШИЙ ЗАПРЕТ: "
                "1) КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО искать заводы-производители, фабрики, изготовители или моно-дилеров одного бренда. Заводы продают только свой товар и не ищут поставщиков. "
                "2) КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО искать конкретные узкие товарные артикулы или марки (например 'насос км 80', 'кабель ввгнг', 'муфта 1квтп', 'арматура dendor'). "
                "3) ЗАПРЕЩЕНО повторять общие фразы 'поставщики оптом', 'участники тендеров 44-фз'. "
                "ОБЯЗАТЕЛЬНЫЕ ШАБЛОНЫ ЗАПРОСОВ: "
                "- 'комплексное снабжение предприятий ТМЦ', 'оператор материально-технического снабжения МТО', 'комплектация промышленных и строительных объектов оборудованием под ключ', 'торговый дом комплексные поставки по ТЗ', 'поставка промышленного оборудования по техническому заданию заказчика', 'реестр участников закупок 44-ФЗ поставка оборудования', 'поставки для госучреждений и бюджетных организаций 44-ФЗ под ключ'. "
                "- Обязательно комбинируй со специализированными направлениями снабжения (энергетика, инженерные сети, нефтегаз, строительство, горнорудная отрасль, ЖКХ) и регионами РФ (Урал, Поволжье, Сибирь, Юг, СЗФО, ДВФО). "
                "- В конце каждого запроса добавляй операторы исключения: -завод -фабрика -производство -изготовитель -\"банковская гарантия\" -\"обучение\" -\"семинар\" -\"эцп\" -\"агрегатор\" -\"курсы\". "
                f"Сгенерируй от {target_q_count} разнообразных целевых поисковых запросов в виде JSON массива строк."
            )
        else:
            system_prompt = (
                "Ты ведущий эксперт по поиску компаний комплексного снабжения, торговых домов и операторов закупок по 44-ФЗ/223-ФЗ. "
                "Целевая аудитория — компании комплексного снабжения, торговые дома, комплектовщики объектов по ТЗ заказчиков. "
                "СТРОГИЕ ПРАВИЛА: "
                "1) НЕ ищи заводы-производители и фабрики. "
                "2) Формируй запросы на комплексное снабжение, комплектацию объектов, торговые дома, поставки по ТЗ для 44-ФЗ / 223-ФЗ. "
                "3) Добавляй отрицательные операторы: -завод -фабрика -производство -изготовитель -\"банковская гарантия\" -\"обучение\" -\"семинар\" -\"эцп\" -\"агрегатор\" -\"курсы\". "
                f"Сгенерируй от {target_q_count} поисковых запросов в виде JSON массива строк."
            )
        user_prompt = f"Целевой профиль (компании комплексного снабжения и торговые дома):\n{prompt}\n\nКоличество контактов: {count}. Сгенерируй {target_q_count} поисковых запросов в виде JSON массива:"
    elif is_extend or wave_index > 1:
        system_prompt = (
            f"Ты ведущий эксперт по поиску реальных B2B поставщиков, дистрибьюторов и оптовых компаний. "
            f"ЭТО РЕЖИМ ДОБОРА (ВОЛНА №{wave_index}). В базе уже собрано {existing_count}+ базовых организаций. "
            "СТРОГИЕ ПРАВИЛА ДОБОРА: "
            "1) СТРОГО ЗАПРЕЩЕНО повторять общие шаблонные фразы. "
            "2) Разбей тему на направления оптовых поставок и дистрибуции. "
            "3) Привязывай запросы к региональным центрам сбыта РФ. "
            "4) Используй отрицательные операторы: -'банковская гарантия' -'обучение' -'семинар' -'эцп' -'агрегатор' -'курсы'. "
            f"Сгенерируй от {target_q_count} глубоких специализированных поисковых запросов в виде JSON массива строк."
        )
        user_prompt = (
        ...
        system_prompt = (
            "Ты ведущий эксперт по поиску реальных B2B поставщиков и коммерческих организаций. "
            "Твоя задача — составить глубокую и точную матрицу поисковых запросов для Яндекса, "
            "которая найдет официальные сайты реальных поставщиков по заданной теме пользователя. "
            "Используй отрицательные операторы Яндекса для исключения мусора: -'банковская гарантия' -'обучение' -'семинар' -'эцп' -'агрегатор' -'курсы'. "
            f"Сгенерируй от {target_q_count} разнообразных целевых поисковых запросов в виде JSON массива строк."
        )
    ...
    if is_tender_or_trading:
        regions = ["Екатеринбург", "Казань", "Самара", "Новосибирск", "Нижний Новгород", "Ростов-на-Дону", "Краснодар", "Уфа", "Челябинск", "Пермь", "Красноярск", "Воронеж", "СПб", "Ярославль", "Россия"]
        types = [
            "комплексное снабжение предприятий ТМЦ 44-фз",
            "оператор материально-технического снабжения и комплектации",
            "торговый дом комплексные поставки оборудования по ТЗ",
            "поставка оборудования по техническому заданию 223-фз",
            "отдел комплектации и снабжения объектов под ключ",
            "участник закупок поставщик промышленного оборудования 44-фз",
        ]
        fallback = []
        for reg in regions:
            for t in types:
                q_str = f"{t} {reg} -завод -фабрика -производство -изготовитель -\"банковская гарантия\" -\"обучение\""
        ...
        types = [
            "торговый дом оптовый поставщик",
        ...
        types = [
            "дистрибьютор оптовые поставки",
            "торговый дом поставщик прайс",
            "комплексное снабжение предприятий",
            "оптовые поставки склад",
        ]
...
async def ai_rerank_outreach_candidates(
    ...
    if is_tender_or_trading:
        system_prompt = (
            "Ты профессиональный AI-аудитор B2B компаний и операторов снабжения. "
            "Твоя задача — отобрать из поисковой выдачи сайты КОМПАНИЙ КОМПЛЕКСНОГО СНАБЖЕНИЯ, ТОРГОВЫХ ДОМОВ, ОПЕРАТОРОВ МТО И ПОДРЯДЧИКОВ ПО 44-ФЗ / 223-ФЗ, закупающих разнообразную номенклатуру по ТЗ заказчиков. "
            "СТРОГО ОТКЛОНЯЙ (is_supplier: false): "
            "- Заводы-производители, фабрики, изготовители собственной продукции (они продают только свой товар со склада и не ищут поставщиков под ТЗ). "
            "- Монобрендовые официальные дилеры одного завода/марки. "
            "- Розничные интернет-магазины одиночных запчастей/товаров. "
            "- Банки, финансовые услуги, лизинг, банковские гарантии. "
            "- Обучающие центры, курсы, семинары, учебные заведения. "
            "- Сервисы электронной подписи (ЭЦП), ОФД, бухгалтерский софт. "
            "- Тендерные агрегаторы, юридические фирмы, доски объявлений, каталоги сайтов. "
            "ПРИНИМАЙ (is_supplier: true, confidence >= 50): "
            "- Компании комплексного снабжения предприятий, операторы МТО, отделы комплектации объектов под ключ. "
            "- Многопрофильные торговые дома, дистрибьюторы комплексных поставок оборудования и ТМЦ по ТЗ заказчика. "
            "- Подрядчики и участники торгов по 44-ФЗ/223-ФЗ. "
            "Верни ТОЛЬКО JSON массив объектов:\n"
            "[{\"index\": 0, \"is_supplier\": true, \"confidence\": 85, \"reason\": \"Компания комплексного снабжения / торговый дом\"}]"
        )
    else:
        system_prompt = (
            ...
            "- Компании, соответствующие профилю запроса пользователя. "
            "Верни ТОЛЬКО JSON массив объектов:\n"
...
async def ai_review_outreach_lead(
    ...
    if is_tender_or_trading:
        system_prompt = (
            "Ты строгий B2B аудитор компаний комплексного снабжения, торговых домов и операторов закупок. "
            "Цель: отобрать организации, которые занимаются комплексным снабжением, оптовой торговлей, подбором оборудования по ТЗ и исполнением контрактов по 44-ФЗ / 223-ФЗ. "
            "СТРОЖАЙШИЕ КРИТЕРИИ: "
            "1. ОТКЛОНЯЙ (is_relevant: false, score < 40): "
            "   - Заводы, фабрики, производственные предприятия, изготовители собственной продукции (они продают только свой товар со склада и не ищут поставщиков под ТЗ). "
            "   - Монобрендовые официальные дилеры/представители одного завода. "
            "   - Розничные интернет-магазины. "
            "   - Банки, гарантии, обучение, ЭЦП, софт, тендерные брокеры. "
            "2. ПРИНИМАЙ (is_relevant: true, score >= 60): "
            "   - Компании комплексного снабжения предприятий, операторы МТО, отделы комплектации объектов под ключ. "
            "   - Многопрофильные торговые дома, дистрибьюторы комплексных поставок оборудования и ТМЦ по ТЗ заказчика. "
            "   - Подрядчики и участники торгов по 44-ФЗ/223-ФЗ. "
            ...
            "  \"score\": 0-100,\n"
            "  \"site_type\": \"trader|supplier|distributor|contractor|manufacturer|unrelated\",\n"
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            if is_tender_or_trading and site_type in ("manufacturer", "unrelated"):
                is_rel = False
