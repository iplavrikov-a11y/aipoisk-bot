...
# This baseline is deliberately dated. Procurement rules change often, so the
# model must not present it as a timeless rule after the next legal update.
PROCUREMENT_LEGAL_BASELINE_AS_OF = dt.date(2026, 7, 12)
PROCUREMENT_LEGAL_BASELINE = """Актуальная правовая база для проверки (срез на 12.07.2026; не выдавай её за более позднюю редакцию):
- Федеральный закон от 28.11.2025 № 425-ФЗ изменил общую ставку НДС с 20% на 22% с 01.01.2026. Для реализации с 01.01.2026 общей ставкой является 22%; расчётная ставка - 22/122.
- Указание 20% в документации само по себе не является ошибкой: оно может относиться к реализации до 01.01.2026 или к переходной ситуации. Не называй 20% нарушением, пока документально не установлены дата реализации в 2026 году и отсутствие специального или переходного основания. Для ставок 0%, 10%, 5% и 7% также сначала установи применимое основание.
- По национальному режиму применяй Постановление Правительства РФ от 23.12.2024 № 1875 только с учётом конкретного товара, кода/перечня, даты и текста извещения. Реестровая запись может быть способом подтверждения происхождения при запрете или ограничении, но нельзя выводить ответ о ней только из названия меры.
- В строке о выписке/реестровой записи пиши **Да** только при прямом требовании в извещении, документации или приложенной норме; **Нет** - только при прямом отсутствии требования либо при прямо указанном неприменении меры; иначе **Не указано**. Обязательно отделяй цитату из документа от правового вывода и указывай, что нужно уточнить при неполных данных."""


def with_procurement_legal_baseline(prompt: str) -> str:
    """Append the dated legal guardrails even when an admin customizes prompts."""
    value = str(prompt or "").strip()
    return f"{value}\n\n{PROCUREMENT_LEGAL_BASELINE}" if value else PROCUREMENT_LEGAL_BASELINE
...
- Не выводи `Да` или `Нет` по выписке/реестровой записи только из вида меры. При запрете и ограничении способ подтверждения происхождения зависит от применимой позиции и текста извещения.
- Пиши `Да` только при прямом требовании выписки, номера или реестровой записи; `Нет` - только когда требование прямо исключено или мера прямо не применяется; в остальных случаях пиши `Не указано` и сформулируй вопрос заказчику.
...
- НДС:
  - для любого правового режима: укажи, включен ли НДС, приведи короткую цитату формулировки из документа и отдели её от правового вывода;
  - не называй 20% ошибкой только потому, что анализ выполняется в 2026 году: сначала проверь дату реализации, переходное или специальное основание ставки;
  - для 223-ФЗ/коммерческой закупки дополнительно укажи риск для УСН.
...
- по нацрежиму отчёт выводит `Да` или `Нет` по выписке/реестровой записи Минпромторга без прямого основания в извещении, документации или приложенной норме; одно название запрета, ограничения или преимущества не является достаточным основанием для такого ответа;
...
- по НДС отчёт называет 20% нарушением только по дате анализа, не установив дату реализации и переходное/специальное основание, либо называет 20% действующей общей ставкой для реализации с 01.01.2026 без такого основания.
...
NATIONAL_REGIME_LINES = {
    "advantage": "- Нацрежим: **Установлено преимущество в отношении товаров российского происхождения.**",
    "restriction": "- Нацрежим: **Действует ограничение закупок товаров.**",
    "prohibition": "- Нацрежим: **Действует запрет закупок товаров.**",
    "not_applied": "- Нацрежим: **Запрет/ограничение указан, но не применяется по обоснованию невозможности соблюдения.**",
}
...
async def generate_procurement_report(settings: SystemSettings, document_text: str) -> ReportGenerationResult:
    ...
    system_prompt = with_procurement_legal_baseline(system_prompt)
...
async def verify_report(
    ...
    prompt = with_procurement_legal_baseline(
        str(report_settings.get("verification_prompt") or DEFAULT_VERIFICATION_PROMPT)
    )
...
def normalize_national_regime_conditions(report: str, source_text: str = "") -> str:
    ...
    natregime_line = NATIONAL_REGIME_LINES[regime_type]
    statement_line = _registry_requirement_line(source_text, regime_type)
    return _replace_national_regime_conditions_block(value, natregime_line, statement_line)


def _registry_requirement_line(source_text: str, regime_type: str) -> str:
    status = _registry_requirement_status(source_text, regime_type)
    suffix = ""
    if status == "Не указано":
        suffix = "; уточните требование у заказчика по применимой позиции"
    return f"- Требуются ли выписки из реестра Минпромторга: **{status}**{suffix}"


def _registry_requirement_status(source_text: str, regime_type: str) -> str:
    """Use only explicit procurement evidence; a measure type is not enough."""
    positive = False
    negative = False
    for raw_line in str(source_text or "").splitlines():
        line = _normalize_fact(raw_line)
        if not any(token in line for token in ("реестр", "реестров", "минпромторг")):
            continue
        if re.search(
            r"\bне\s+(?:требу\w*|нужн\w*|обязательн\w*|предоставля\w*|представля\w*)\b",
            line,
        ) or "отсутствует необходимость" in line:
            negative = True
            continue
        if re.search(
            r"\b(?:требу\w*|необходим\w*|обязательн\w*|предостав\w*|представ\w*|указ\w*|подтвержда\w*)\b",
            line,
        ):
            positive = True
...
def _extract_targeted_national_regime_requirement_types(text: str) -> set[str]:
    ...
    for match in re.finditer(
        r"\bустановлено\s+[-–—•\s]*(запрет|ограничени(?:е|я|й)|преимуществ)[^\n]{0,1200}",
        text,
        flags=re.IGNORECASE,
    ):
        line_start = text.rfind("\n", 0, match.start()) + 1
        if re.search(r"\bесли\b", text[line_start : match.start()], flags=re.IGNORECASE):
            continue
        blocks.append(match.group(0))
