...
- отчет изменил буквальное значение карточечного поля официального источника/ЕИС/электронной площадки: "Способ осуществления закупки", заказчик, ИНН/КПП, НМЦК, площадка, срок подачи заявок или дата итогов;
...
- отчет вывел числовой код источника как "Способ закупки", хотя в извещении/документации есть человекочитаемый способ закупки;
- отчет взял технический timestamp структурированного источника по итогам, хотя в извещении есть явная дата/время рассмотрения, оценки и подведения итогов;
...
DEFAULT_OFFICIAL_CARD_REPAIR_PROMPT = """Исправь отчет по официальной карточке закупки / ЕИС / электронной площадки.
