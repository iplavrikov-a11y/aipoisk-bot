...
1. Выдели целевые закупаемые позиции из профиля закупки (наименование, назначение, ключевой функционал).
2. ПРИНИМАЙ запись из ГИСП, если:
   - Наименование продукции в реестре прямо соответствует закупаемой позиции или ее прямому функциональному аналогу/модели/типоразмеру.
   - Завод производит именно требуемый вид оборудования/товара (учитывай отраслевую терминологию, ГОСТ, ТУ, серии и номенклатурные обозначения производителей).
3. ОТКЛОНЯЙ запись из ГИСП, если:
   - Продукция относится к принципиально иному типу, назначению или сфере применения, не решающей задачу закупки.
   - Вспомогательный инвентарь или несвязанная продукция при закупке основного оборудования/материалов.

Ответ строго JSON:
{{"accepted_indexes":[{{"index":0,"reason":"обоснование соответствия продукции в ГИСП"}}]}}
...
КРИТИЧНО: НЕ выдумывай характеристики и технологии, которых нет в ТЗ (например, не добавляй мойку, сушку или ультразвук, если в ТЗ речь идёт о механической зачистке/снятии заусенцев). Строго опирайся на текст ТЗ.
...
- СТРОГО соблюдай тип и технологическое назначение товара: не подменяй предмет закупки смежными процессами (например, для зачистного станка / снятия заусенцев нельзя искать моечные комплексы, ультразвуковые ванны или окрасочные линии).
...
Не выбирай сайты со смежными технологиями, если они не соответствуют предмету закупки (например: моечные комплексы/ванны, мойки деталей, ультразвук, окрасочные камеры при закупке зачистных/шлифовальных/гратоснимающих станков).
{minprom_guidance}
...
- СТРОГО отклоняй (action: "reject", product_fit: "unrelated"), если сайт предлагает смежный, но принципиально другой технологический процесс (например: моечные машины/ванны, мойки деталей/ДВС, ультразвуковую очистку, дробеструйные/окрасочные линии при закупке зачистных/шлифовальных/гратоснимающих станков или станков для снятия заусенцев).
Ответ строго JSON:
{{
