...
- Lead Generation & Outreach CRM Module (2026-08):
  ...
  - Multi-Tier Anti-Spam Engine & Self-Learning Blacklist: Multi-layer heuristic classifier for incoming IMAP messages (`is_spam_message` in `backend/app/outreach_mail.py`) automatically isolates disposable spam networks (`.shop`, `.pro`, `.life`, `.store`, `rumixos.shop`, `thespacebanana.com`, `prorucane.pro`, `rabbitandjohn.com`, etc.), B2C consumer goods (Makita tools, water heaters), esoteric/love-coach pitches, mass spam services ("разошлем КП"), and numeric Mail.ru accounts sending Google Form links.
  - Blacklist Learning & Management: Marking an email as spam automatically extracts sender domains/rules into `settings.spam_rules_json`, retroactively quarantines matching messages, and exposes rules in `⚙️ Настройки почты` alongside a 1-click `🗑️ Очистить весь спам` purge action in the inbox.
  - Database & Backend: Automatic SQLite schema migrations for `outreach_inbox` (`category`, `is_spam`) and `outreach_settings` (`spam_rules_json`). Enriched `/api/outreach/inbox` with company name, phone, task matching, server-side pagination (`limit`, `offset`, `total`), and safe query handling.
