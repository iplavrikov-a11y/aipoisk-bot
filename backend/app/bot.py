...
BOT_SHORT_DESCRIPTION = "AI-бот для тендеров: поставщики, анализ документации и отчёты по закупкам."
BOT_DESCRIPTION = (
    "AI Poisk помогает по тендерам: ищет и проверяет поставщиков по ТЗ, "
    "анализирует закупочную документацию и выдаёт готовые файлы в Telegram.\n\n"
...
def _start_text() -> str:
    return (
        ...
        "3. Отправьте ТЗ, документацию, архив или ссылку, если режим её поддерживает.\n"
...
async def create_button(message: Message) -> None:
    await message.answer(
        "Выберите тип отчёта.\n\n"
        "Если не уверены, начните с «Поставщики по одному ТЗ» для одного файла или с «Анализ документации» для комплекта закупки.",
...
async def help_button(message: Message) -> None:
    await message.answer(
        ...
        "📄 «Анализ документации» — комплект документации, архив или ссылка на закупку.\n"
        "📄🔎 «Анализ + поставщики» — анализ документации и отдельный Excel с поставщиками.\n\n"
