...
5. ЕСЛИ ПАРАМЕТР ЯВЛЯЕТСЯ ЗАКАЗНОЙ ОПЦИЕЙ ИЛИ ОТСУТСТВУЕТ В ОТКРЫТЫХ КАТАЛОГАХ:
...
6. ИДЕНТИФИКАЦИЯ БАЗОВОГО ТОВАРА И АНАЛОГОВ:
   - "identified_brand" / "manufacturer": Честно укажи реальный выявленный бренд, модель и завод-изготовитель (отечественный или зарубежный, если в ТЗ заложен импорт). Категорически запрещено выдумывать неподходящие заводы РФ!
   - ДЛЯ АНАЛОГОВ (alternative_brands): Указывай реально существующие модели российских заводов (в т.ч. из Реестра Минпромторга РФ ГИСП). Построчно заполни характеристики аналога на основе его каталога/паспорта. Если параметр соответствует ТЗ — ставь "match", если отличается — "mismatch", если неизвестен — "clarify". Если российские аналоги имеют критические расхождения (несовместимость сред, габаритов, мощности), честно укажи статус отклонения (mismatch).
...
3. Извлеки 3-5 КЛЮЧЕВЫХ ТЕХНИЧЕСКИХ ПАРАМЕТРОВ из ТЗ (тип прибора/изделия, производительность, мощность, габариты, давление, среда, ГОСТ/ТУ или артикул).
4. Сформируй список 2-5 ОТРИЦАТЕЛЬНЫХ КЛЮЧЕВЫХ СЛОВ (negative_keywords) — неподходящие технологии или товары-антагонисты.
5. Сформируй 4-6 высокоточных поисковых запросов для Яндекса (с паспортами, каталогами, техническими описаниями и опросными листами).

КРИТИЧЕСКИЕ ПРАВИЛА:
- СТРОГО ЗАПРЕЩЕНО выдумывать конкретные модели или индексы из головы, если точная модель не названа в самом ТЗ!
- Поисковые запросы должны опираться на реальные ключевые параметры из ТЗ, чтобы найти именно то оборудование/паспорт, которое заложено заказчиком.
