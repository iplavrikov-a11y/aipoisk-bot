...
- Isolated Admin Rerun & Multi-File Expert Task Supplement Workflow (2026-09):
  - Non-Destructive Admin Reruns (`parent_job_id`, `is_admin_rerun` in `backend/app/models.py`, `backend/app/main.py`, `frontend/src/App.tsx`): Triggering a rerun via the Play button in the admin panel creates an isolated administrator task with `[Админ]` prefix and a distinct `👑 Админ-доработка` card badge, maintaining full linkage to the client's root task without overwriting or deleting original customer files.
  - Multi-File Supplement Selection & 1-Click Direct File Inspection (`GET /api/jobs/{job_id}/supplement-candidates`, `AdminSupplementModal`): Administrators can select multiple output files (e.g. suppliers Excel database + RFQ Word docx) across both original and admin rerun executions to deliver simultaneously to the client's cabinet and Telegram. Each candidate row includes an instant `[📥 Открыть]` preview / download button for immediate file inspection before delivery.
  ...
  - Full Test Suite Verification: 658/658 passing backend tests (`backend/tests/test_admin_supplement.py`) and live production deployment verified via `./scripts/deploy_tenderlex_live.sh`.
