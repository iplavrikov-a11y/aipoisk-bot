...
- For Telegram bot behavior changes, also verify that `./scripts/deploy_tenderlex_live.sh` actually restarted `tenderlex-bot.service`. If the script prints `skipping api/worker/bot restart because active jobs are present`, wait for active `pending`/`running` jobs to clear and rerun the deploy or otherwise verify a fresh bot service start before reporting the bot change as live.
