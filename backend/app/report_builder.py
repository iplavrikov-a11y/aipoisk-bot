...
def _parse_quote_markdown_for_sheet(markdown: str, fallback_title: str) -> tuple[str, list[dict], dict[str, str]]:
    ...
    for line in lines:
        ...
        if not s:
            continue
        if s.startswith("**") and s.endswith("**") and len(s) > 4 and not title:
            title = s[2:-2].strip()
        elif s.startswith("### Условия поставки"):
            in_table = False
        elif s.startswith(("- **", "* **", "- ", "* ")):
            m = re.match(r"^[-*]\s*(?:\*\*)?([^*:]+)(?:\*\*)?\s*[:\-]\s*(.+)$", s)
            if m:
                k, v = m.group(1).strip(" *:-"), m.group(2).strip()
                if v and v != "Просим указать в КП":
                    conditions[k] = v
    ...
    if len(table_lines) >= 3:
        ...
        headers = [_normalize_table_header(c) for c in raw_headers]
        col_name = None
        col_char = None
        col_unit = None
        col_qty = None
        for i, h in enumerate(headers):
            if any(k in h for k in ("наименование", "товар", "позиция", "предмет")):
                if col_name is None:
                    col_name = i
            elif any(k in h for k in ("характерист", "описание", "требован", "параметр")):
                if col_char is None:
                    col_char = i
            elif any(k in h for k in ("ед.изм", "единица", "ед изм", "ед.")):
                if col_unit is None:
                    col_unit = i
            elif any(k in h for k in ("кол-во", "количество", "объем", "кол.")):
                if col_qty is None:
                    col_qty = i
        ...
        for row_str in table_lines[2:]:
            ...
            if not name or re.fullmatch(r"[:\-\s|]+", name):
                continue
            char_val = cells[col_char] if col_char is not None and col_char < len(cells) else ""
            char_clean = char_val.replace("<br>", "\n").replace("<br/>", "\n").replace("<br />", "\n")
            unit_val = cells[col_unit] if col_unit is not None and col_unit < len(cells) else "Просим указать в КП"
            qty_val = cells[col_qty] if col_qty is not None and col_qty < len(cells) else "Просим указать в КП"
            items.append(
                {
                    "num": str(len(items) + 1),
                    "name": name,
                    "characteristics": char_clean,
                    "unit": unit_val,
                    "quantity": qty_val,
                }
            )

    return title or fallback_title, items, conditions


def _write_quote_request_sheet(
    wb: Workbook,
    *,
    title: str,
    subject: str = "",
    ...
    extracted_title, items, conditions = _parse_quote_markdown_for_sheet(quote_markdown, base_title)
    final_title = extracted_title or base_title

    # Шапка
    ws.append(["TenderLex | ГОТОВЫЙ ТЕКСТ ЗАПРОСА КП ДЛЯ КОПИРОВАНИЯ (НАЖМИТЕ НА ЯЧЕЙКУ A3 И СКОПИРУЙТЕ CTRL+C)"])
    _style_range(ws, 1, 1, 5, font=Font(name="Calibri", size=13, bold=True, color="047857"), align=Alignment(vertical="center"))
    ws.row_dimensions[1].height = 26

    ws.append([f"Предмет закупки / номенклатура: {final_title}"])
    _style_range(ws, 2, 1, 5, font=Font(name="Calibri", size=11, bold=True, color="064E3B"), align=Alignment(vertical="center"))
    ...
    if items:
        for it in items:
            q_part = f" — {it['quantity']} {it['unit']}" if it['quantity'] != "Просим указать в КП" and it['unit'] != "Просим указать в КП" else ""
            char_part = ""
            if it['characteristics'] and it['characteristics'] != "Просим указать в КП":
                char_lines = [l.strip(" •-") for l in it['characteristics'].split("\n") if l.strip()]
                if char_lines:
                    char_summary = "; ".join(char_lines[:4])
                    if len(char_lines) > 4:
                        char_summary += f" и др. (всего {len(char_lines)} параметров)"
                    char_part = f"\n   Характеристики: {char_summary}"
            items_text_parts.append(f"• {it['name']}{q_part}{char_part}")
    else:
        items_text_parts.append(f"• {final_title} (согласно техническому заданию / спецификации заказчика)")

    items_block = "\n".join(items_text_parts)

    conditions_text_parts = []
    for k, v in conditions.items():
        if v and v != "Просим указать в КП":
            conditions_text_parts.append(f"- {k}: {v}")

    conditions_block = ""
    if conditions_text_parts:
        conditions_block = "\n\nУсловия поставки:\n" + "\n".join(conditions_text_parts)

    quote_text = (
        f"Тема письма: Запрос коммерческого предложения / счёта на поставку: {final_title}\n\n"
        "Добрый день!\n\n"
    ...
    line_count = len(quote_text.split("\n"))
    ws.row_dimensions[3].height = max(180, min(500, line_count * 15 + 20))

    # Разделитель
    ws.append([None] * 5)
    ws.row_dimensions[4].height = 12

    # Спецификация товаров таблицей
    ws.append(["СПЕЦИФИКАЦИЯ ТОВАРОВ И ТРЕБОВАНИЙ ПО ТЗ"])
    _style_range(ws, 5, 1, 5, font=Font(name="Calibri", size=11, bold=True, color="064E3B"), align=Alignment(vertical="center"))
    ws.row_dimensions[5].height = 22

    # Заголовки таблицы спецификации
    spec_headers = ["№", "Наименование товара", "Характеристики и требования", "Ед. изм.", "Кол-во"]
    ...
    for col_idx, h in enumerate(spec_headers, start=1):
        ...
        cell.alignment = Alignment(wrap_text=True, vertical="center", horizontal="center" if col_idx in (1, 4, 5) else "left")
    ...
    current_row = 7
    rows_to_render = items or [
        {
            "num": "1",
            "name": final_title,
            "characteristics": "Требования согласно техническому заданию заказчика",
            "unit": "Просим указать в КП",
            "quantity": "Просим указать в КП",
        }
    ]

    for idx, it in enumerate(rows_to_render, start=current_row):
        is_even = ((idx - current_row) % 2 == 1)
        ...
        ws.append([it["num"], it["name"], it["characteristics"], it["unit"], it["quantity"]])

        c1 = ws.cell(row=idx, column=1)
        c1.alignment = Alignment(vertical="top", horizontal="center")
        c1.font = Font(name="Calibri", size=10, color="0F172A")
        c1.fill = base_bg
        c1.border = thin_border

        c2 = ws.cell(row=idx, column=2)
        c2.alignment = Alignment(vertical="top", horizontal="left", wrap_text=True)
        c2.font = Font(name="Calibri", size=10, bold=True, color="0F172A")
        c2.fill = base_bg
        c2.border = thin_border

        c3 = ws.cell(row=idx, column=3)
        c3.alignment = Alignment(vertical="top", horizontal="left", wrap_text=True)
        c3.font = Font(name="Calibri", size=9.5, color="334155")
        c3.fill = base_bg
        c3.border = thin_border

        c4 = ws.cell(row=idx, column=4)
        c4.alignment = Alignment(vertical="top", horizontal="center")
        c4.font = Font(name="Calibri", size=10, color="0F172A")
        c4.fill = base_bg
        c4.border = thin_border

        c5 = ws.cell(row=idx, column=5)
        c5.alignment = Alignment(vertical="top", horizontal="center")
        c5.font = Font(name="Calibri", size=10, bold=True, color="0F766E")
        c5.fill = base_bg
        c5.border = thin_border

        ws.row_dimensions[idx].height = _calc_supplier_row_h([(it["name"], 32), (it["characteristics"], 65)], min_h=24)
        current_row = idx + 1

    # Условия поставки
    if conditions_text_parts:
        current_row += 1
        ws.cell(row=current_row, column=1, value="УСЛОВИЯ ПОСТАВКИ И ТРЕБОВАНИЯ К ПОСТАВЩИКУ")
        _style_range(ws, current_row, 1, 5, font=Font(name="Calibri", size=11, bold=True, color="064E3B"), align=Alignment(vertical="center"))
        ws.row_dimensions[current_row].height = 22
        current_row += 1

        for label, val in conditions.items():
            if not val or val == "Просим указать в КП":
                continue
            ws.cell(row=current_row, column=1, value=label)
            c_lbl = ws.cell(row=current_row, column=1)
            c_lbl.font = Font(name="Calibri", size=10, bold=True, color="0F172A")
            c_lbl.fill = PatternFill(start_color="F8FAFC", end_color="F8FAFC", fill_type="solid")
            c_lbl.border = thin_border
            c_lbl.alignment = Alignment(vertical="center", horizontal="left")

            ws.cell(row=current_row, column=2, value=val)
            c_val = ws.cell(row=current_row, column=2)
            c_val.font = Font(name="Calibri", size=10, color="334155")
            c_val.fill = PatternFill(start_color="FFFFFF", end_color="FFFFFF", fill_type="solid")
            c_val.border = thin_border
            c_val.alignment = Alignment(vertical="center", horizontal="left", wrap_text=True)
