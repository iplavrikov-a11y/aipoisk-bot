...
- Lead Generation & Outreach CRM Module (2026-08):
  ...
  - Database & Backend: Automatic SQLite schema migrations for `outreach_inbox` (`category`, `is_spam`) and `outreach_campaigns` (`selected_lead_ids`, `audience_type`) in `backend/app/db.py`. Enriched `/api/outreach/inbox` with company name, phone, and task matching. Fixed AI generation endpoint with system settings and tier integration.
